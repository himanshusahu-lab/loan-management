...
export class idsutil {
  ...
  async getIDSClientInstance(parentSpanInst, clientInstance = null, ...others) {
    ...
    try {
      ...
      bh = await this.sd_2HUsDu8nJiWU4BRr(bh, parentSpanInst);
  ...
  async sd_2HUsDu8nJiWU4BRr(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_2HUsDu8nJiWU4BRr',
      parentSpanInst
    );
    try {
      ...
      //appendnew_next_sd_2HUsDu8nJiWU4BRr
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_NDvR3YN7qjsaaO0E(bh, parentSpanInst) {
    ...
    try {
      if (
        ...
        bh = await this.sd_j2cylrv1HZXnQha6(bh, parentSpanInst);
      }
      this.tracerService.sendData(spanInst, bh);
  ...
  async sd_J9L3oEbqPtl8tfbi(bh, parentSpanInst) {
    ...
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      bh = await this.sd_JvlHmjPRoQ5VnBB3(bh, parentSpanInst);
  ...
  async sd_JvlHmjPRoQ5VnBB3(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_JvlHmjPRoQ5VnBB3',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_JvlHmjPRoQ5VnBB3
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  async sd_j2cylrv1HZXnQha6(bh, parentSpanInst) {
    const spanInst = this.tracerService.createSpan(
      'sd_j2cylrv1HZXnQha6',
      parentSpanInst
    );
    try {
      ...
      this.tracerService.sendData(spanInst, bh);
      //appendnew_next_sd_j2cylrv1HZXnQha6
      return bh;
    } catch (e) {
      return await this.errorHandler(
        bh,
  ...
  private async errorHandler(
    ...
    if (
      false ||
      (await this.sd_S5IEumEmQC2v4rYG(bh, parentSpanInst))
  ...
  async sd_S5IEumEmQC2v4rYG(bh, parentSpanInst) {
    const nodes = ['handleTokenExpiry'];
    if (nodes.includes(bh.errorSource)) {
      bh = await this.sd_EgUC5B8p6Vp4y70s(bh, parentSpanInst);
      //appendnew_next_sd_S5IEumEmQC2v4rYG
      return true;
    }
    return false;
  }
